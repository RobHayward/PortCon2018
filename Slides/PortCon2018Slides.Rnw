...
\begin{frame}{BoA: calm and crisis}
	This will be a BoA chart with price and returns
\end{frame}
...
% main blocks
\node at (6, 7) [block] {Swap Counterparty};  
...
\includegraphics[scale = 0.9]{tyne_bridge}
\end{frame}
...
\begin{frame}{ESG Pressure}
	California has introduced legislation to require funds (Calpers and Calsters) to explain how they are managing climate risk
	\pause
	\begin{block}{}
...
\begin{itemize}[<+-| alert@+>]
	...
	\item International Capital Market Association (ISMA) has set out some guidelines for bonds that can be labelled \emph{green}
		\begin{itemize}
			...
			\item reporting
		\end{itemize}
	\item The Climate Bond Initiative: certify, advise and promote
	\item There is increased competition from the issuance of social uses of funds
		...
		\begin{itemize}
			\item passive strategies require active decision-making
				\begin{itemize}
					...
					\item benchmarking market portolio and liabilities
				\end{itemize}
			\item active cost of active investment can be reduced with more systematic focus
				\begin{itemize}
					\item use of ETF
					\item identifcation of active factors
				\end{itemize}
		\end{itemize}
