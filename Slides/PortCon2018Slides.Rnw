...
\tikzstyle{shape1} = [circle, minimum width = 3.0cm, fill = brown!40, text centered, 
...
\end{center}
\end{table}
This gives most likely growth estimates and rsks to that forecasts based on the probability that there is a regime-switch
\end{frame}
...
da$Date <- as.Date(da$Date, format = "%Y-%m-%d")
  plot(da$Date, da[,5], type = 'l', main = "MAGA Performance", col = 'chocolate4', 
     xlab = "Year", ylab = "Price")
@
\end{frame}
...
\includegraphics[scale = 0.2]{tyne_bridge}
\end{frame}
