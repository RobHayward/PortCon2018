...
\tikzstyle{block} = [rectangle, rounded corners, minimum width=3cm, 
  minimum height= 2cm, text centered, text width = 2.8cm, draw = black, fill = brown!30]
\tikzstyle{shape1} = [circle, minimum width = 3.0cm, fill = brown!30, text centered, 
...
\begin{frame}{Emerging Economies}
...
\item 830\% of European fund managers believed that is was becoming more important (Shroders survey June 2018)
\item Problems
\begin{itemize}
\item benchmarking
...
\begin{itemize}[<+-| alert@+>]
	...
	\item Deloitte estimate that \$24tn will be transferred to millennias in the next 15 years
...
\begin{frame}{Turbulence: climate}
	Investors are paying increased attention to risks linked to fossil fuels
	\pause
\begin{itemize}[<+-| alert@+>]
	\item Which companies benefit from changes in regulation and consumer demand
	\item BlackRock, BNP Paribas and StateStree have ETF that hold low-carbon footprint companies
	\item Many funds also look forward to those firms that will benefit in the future
	\item Risk reduction and opportunity
\end{itemize}
\end{frame}
